...
class SleepySheepy(Enemy):
    ...
    def update(self, step):
        if (self == SleepySheepy.leadSheep):
            self.velocity.rotate(random.uniform(-10, 10))
            self.position += self.velocity.normalize() * step * 60
        elif (type(SleepySheepy.leadSheep) is SleepySheepy) :
            print("Reassigning: Sleepy Sheepy Position = ", SleepySheepy.position)
            self.velocity = (SleepySheepy.position - self.position).normalize()
        self.orientation = -self.velocity.calculateVectorAngle()

    def updateFlock(self, other):
